...
export const getChannel_List = async (
  accessToken: string
): Promise<ChannelList[]> => {
...
export const getChannel_Info = async (
  accessToken: string,
  channelId: string
): Promise<ChannelInfo> => {
...
export const getChannel_Summary = async (
  accessToken: string,
  channelId: string
  ...
  try {
    const response = await axios.get(
      `${channel_summary_URL}?cid=${channelId}`,
      {
        headers: {
          Authorization: 'datalake',
        },
...
export const getChannel_Daily = async (
  accessToken: string,
  channelId: string
): Promise<ChannelDaily> => {
  ...
  try {
    ...
    if (response && response.status === 200) {
      //채널 통계 정보
      const view: number[] = response.data.map(
        (channelStatistics: any) => channelStatistics.total_views
      );
      const subscriber: number[] = response.data.map(
        (channelStatistics: any) => channelStatistics.total_subscribers
      );
      const est: number[] = response.data.map(
        (channelStatistics: any) => channelStatistics.est_partner_rev
      );
